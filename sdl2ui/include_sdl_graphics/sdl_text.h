...
public:
	Ctext( Cdialog *parent, const Crect &rect, keybutton code, const Sfont &font,
		   ...
		   colour text=0, Egravity align=GRAVITY_CENTER,
		   int cursor=-1, int background=0);
	Ctext( Cdialog *parent, const Crect &rect, keybutton code, const Sfont &font,
		   textId value, colour text=0, Egravity align=GRAVITY_CENTER);
	virtual ~Ctext();
	virtual void onPaint( int touch);
	void 	onPaintText( const std::string &text, int touch);
    ...
    bool doesNotFit() { return(m_doesNotFit); }
    bool isChineseText() { return(m_isChineseText); }
	void decreaseFont();
...
private:
    ...
    bool            m_doesNotFit;
    bool            m_isChineseText;
    Eresize         m_canDecreaseFont;
};

typedef std::shared_ptr<Ctext> CtextPtr;
